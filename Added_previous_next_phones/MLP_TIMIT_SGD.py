...
# you give an integer and it gives you a onehot vector
def onehot(x,max):
	onehot = np.zeros(max,dtype=theano.config.floatX)
	onehot[x] = 1
	return onehot

# a simple home-made algo to find quickly the previous phone location
def previous(list, index):
	...
	while ((previous_value == value) and (previous_index >= 101)):
		previous_index -= 100
...
# a simple home-made algo to find quickly the next phone location
def next(list, index):
	...
	while ((next_value == value) and (next_index < len(list)-101)):
		next_index += 100
...
class TIMIT(object):
	...
	# this function basically gives you a batch to train your neural network
	# TODO : pre-process the dataset instead of processing it here
	def get_batch(self):
		...
		for k in xrange(self.batch_size):
			...
			current_phone = self.phones[sentence_id][start+self.n_samples_in]
			batch_x[k][self.n_samples_in:self.n_samples_in+self.num_phones]=onehot(current_phone, self.num_phones)
			
			# get the previous phone and the number of samples after previous phone
			previous_phone, samples_to_previous_phone = previous(self.phones[sentence_id], start+self.n_samples_in)
			batch_x[k][self.n_samples_in+self.num_phones:self.n_samples_in+2*self.num_phones]=onehot(previous_phone,self.num_phones)
			batch_x[k][self.n_samples_in+2*self.num_phones] = samples_to_previous_phone/2000.
			
			# get the next phone and the number of samples before next phone
			next_phone, samples_to_next_phone = next(self.phones[sentence_id], start+self.n_samples_in)
			batch_x[k][self.n_samples_in+2*self.num_phones+1:self.n_samples_in+3*self.num_phones+1]=onehot(next_phone,self.num_phones)
			batch_x[k][self.n_samples_in+3*self.num_phones+1] = samples_to_next_phone/2000.
...
def train(learning_rate, batch_size, n_train_batch, n_valid_batch, n_test_batch, n_samples_in, n_samples_out, n_hidden, n_hidden_layers, load = None):
	...
	while stop < 100 :
		...
		# if validation loss get better
			...
			model.save_params('/u/courbarm/best_params_mlp_800_800_1_epoch_'+str(epoch)+'.pkl')
...
if __name__ == "__main__":
	...
	train(learning_rate=0.003, batch_size = 32, n_train_batch = 100000, n_valid_batch = 10000, n_test_batch = 10000,
			n_samples_in = 160, n_samples_out = 1, n_hidden = 800, n_hidden_layers = 2)#, load = 'best_params_mlp_500_1_epoch_47.pkl')
