...
# Note: if I want to work with GPU, I have to put the dataset into shared variables
class TIMIT(object):
	...
	def get_batch(self):
		...
		batch_x = np.zeros((self.batch_size, self.n_samples_in + self.num_phones), dtype=theano.config.floatX)
...
class HiddenLayer(object):
	...
	def __init__(self, n_in, n_out):
		...
		# shared variables are the state of the builded function
...
def train(learning_rate, batch_size, n_train_batch, n_valid_batch, n_test_batch, n_samples_in, n_samples_out, n_hidden, n_hidden_layers, load = None):
	...
	# creation of model
	model = MLP(n_input = n_samples_in + train_set.num_phones, n_output = n_samples_out, n_hidden = n_hidden, n_hidden_layers = n_hidden_layers)	
	...
	while stop < 10 :
		...
		# if validation loss get better
			...
			model.save_params('best_params_epoch_'+str(epoch)+'.pkl')
...
if __name__ == "__main__":
	...
	train(learning_rate=0.003, batch_size = 32, n_train_batch = 20000, n_valid_batch = 2000, n_test_batch = 2000, 
			n_samples_in = 160, n_samples_out = 1, n_hidden = 300, n_hidden_layers = 1)#, load = 'best_params_epoch_2.pkl' )
